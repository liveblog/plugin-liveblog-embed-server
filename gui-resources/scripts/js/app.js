...
process.on('message', function(liveblog) {
    liveblog.browserUrl = urlHref.browserUrl;
    GLOBAL.liveblog = liveblog;
    ...
    requirejs([
        ...
        utils.dispatcher.once('blog-model.request-failed', function() {
            if (!sent) {
                sent = true;
                process.send({code: 400, body: 'Request for blog has failed.'});
            }
        });
        utils.dispatcher.once('theme-file.request-failed', function() {
            if (!sent) {
                sent = true;
                process.send({code: 400, body: 'Request for theme file has failed.'});
            }
        });
        ...
        layout.blogModel.get('publishedPosts').on('sync', function() {
            if (!sent) {
                sent = true;
                process.send({code: 200, body: layout.render().$el.html()});
            }
        });
    }, function(err) {
        process.send({code: 400, body: err});
    });
});
